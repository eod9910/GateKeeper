#!/usr/bin/env python3
"""
Mirror Cursor session transcripts into this repo.

This reads Cursor's per-session JSONL files from:

    %USERPROFILE%\\.cursor\\projects\\<project-slug>\\agent-transcripts
...
LIVE_TRANSCRIPT_NAME = "cursor-session.md"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", default=None, help="Destination folder for mirrored markdown and raw copies.")
    parser.add_argument(
        "--interval",
        type=float,
    ...
    parser.add_argument("--all", action="store_true", help="Mirror every session in the project. Default: newest only.")
    return parser
...
def mirror_once(agent_dir: Path, output_dir: Path, newest_only: bool) -> dict[str, Any]:
    ...
    for idx, jsonl_path in enumerate(files):
        ...
        if idx == 0:
            ...
            write_text_if_changed(decoded_dir / LIVE_TRANSCRIPT_NAME, markdown)
            legacy_latest_path = decoded_dir / "latest-session.md"
            if legacy_latest_path.exists():
                legacy_latest_path.unlink()
            wrong_user_to_validator_path = decoded_dir / "user-to-validator.md"
            if wrong_user_to_validator_path.exists():
                wrong_user_to_validator_path.unlink()
    ...
    metadata = {
        ...
        "live_markdown": LIVE_TRANSCRIPT_NAME if newest_md_name else None,
...
def main() -> int:
    ...
    output_dir = Path(args.output).resolve() if args.output else (Path(__file__).resolve().parent / "cursor-live")
