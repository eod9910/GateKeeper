...
$agentTranscriptsDir = $env:LIGHTRAG_CURSOR_AGENT_TRANSCRIPTS_DIR
...
if ($matchingProcess) {
  ...
  Write-Output "Cursor transcript mirror already running (PID $($matchingProcess.ProcessId))."
  exit 0
}
...
if ($agentTranscriptsDir) {
  $arguments += @('--project-dir', ('"{0}"' -f $agentTranscriptsDir))
}
