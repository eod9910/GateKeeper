...
- How the four system layers (Research → Signal → Strategy → Portfolio) connect
...
- **Family discovery (research layer only):** `agent-relay/planning-docs/ongoing/family-discovery-v2-prd-pdr.md`
- **Structure-only family test ledger:** `agent-relay/planning-docs/reference/family-structure-validation-ledger.md`
- **Phased roadmap (layers + UI order):** `agent-relay/planning-docs/ongoing/Update.md`
- **Symbolic Regression (formula discovery):** `agent-relay/planning-docs/ongoing/SR Engine.md` — scoring formulas for setups; can use family features after indicator-based first path.
- **Strategy validation rules:** `agent-relay/planning-docs/reference/strategy-validation-policy.md`
- **Other research path (composite/strategy discovery):** `agent-relay/planning-docs/ongoing/research-to-live-trading.md`
...
Do not name patterns. Let the data define recurring **structures** from price (bars → pivots → legs → labels → 5-pivot motifs). Group those motifs into **families** by structural properties. Measure what happens *after* each family appears, across discovery/validation/holdout and across symbols. Only then treat a family as a candidate for trading. Structure is discovered and validated; names are optional later.

**Gap this PRD addresses:**
The research pipeline (families, explorer, stats) exists. The missing piece is a clear plan for how those families are **used** in this repo — not as a standalone tool, but as a new class of signal that flows through the existing scanner, validator, and execution stack.
...
| **2. Signal**   | When does that structure actually occur in real time?     | Not built     | New plugin type: “family signal” emits at pivot-5 confirmation |
...
- **Runs offline.** Script: `backend/scripts/run_atr_pivot_research.py`. Symbols: SPY, QQQ, IWM, DIA (daily, 10y). Pipeline: bars → ATR pivots → legs → labels → 5-pivot motifs → outcomes → family aggregation (v1 exact + v2 generalized) → fragmentation → inspection → cross-symbol comparison → stability.
...
- **Use:** Inspect families, rank them, compare to baselines, decide which families are “real” before building the signal layer.
...
- **Integration:** Registered in `registry.json`, invoked by `strategyRunner.py` like any other plugin. Scanner runs it; copilot sees the candidate. No new “standalone scanner” — the existing scanner runs family-based strategies as one more plugin.
...
| # | Deliverable | Purpose |
|---|-------------|---------|
| 1 | **Family ranking controls** | Sort/filter families by t-score, occurrence count, cross-symbol dispersion, agreement status, candidate status. Answer: “Which families matter most?” |
| 2 | **Baseline / null-model comparison panel** | Compare family outcomes vs random timestamps, random motifs, direction-only baseline. Answer: “Better than what?” |
| 3 | **Visual motif inspection improvements** | Refine inspection so we can confirm: “Do these examples look like the same structural thing?” |

**Exit condition:** You can rank families, compare them to baselines, and inspect them. You have a short list of “validated families” to take to Phase 2.
...
| 6 | **Strategy layer for families** | Wrap family signal with risk_config and exit rules; run through full validator (Tier 1 → 2 → 3). Approve or tombstone per strategy-validation policy. |
...
- **Clarity:** Any contributor can read this PRD and understand that “structural families” are not a separate product — they are a structure-first signal source that flows through the same pipeline as the rest of the app.
...
- **Initiative name:** Structural Families to Execution (or “structure-first trading evolution”).
- **This document:** `ACTIVE/structural-families-to-execution-prd.md` — master PRD for the full evolution.
- **Discovery-only PRD:** `ACTIVE/family-discovery-v2-prd-pdr.md` — research layer and family discovery details only.
