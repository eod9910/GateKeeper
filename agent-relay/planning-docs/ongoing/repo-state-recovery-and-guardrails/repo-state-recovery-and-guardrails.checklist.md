...
**Created:** 2026-04-16
**Status:** ACTIVE
**Source-of-truth role:** companion to `repo-state-recovery-and-guardrails-prd.md` — captures the actions taken on 2026-04-16 and the items that still need a human decision.
...
- **B) Park.** Pick one initiative as the active one, branch the others into `agent-relay/planning-docs/ongoing/`, and do not commit their code into `main` until they are picked up.
...
**No execution required this session** — but the next architectural unit of work should be a small one-pager `agent-relay/planning-docs/reference/ts-python-contract-policy.md` that picks a single rule, e.g.:
