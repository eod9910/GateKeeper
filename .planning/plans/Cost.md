...
**Codebase Version**: [From cursor.md phase status]
...
**Estimated time savings with cursor Code**: [X]%
**Effective hourly rate with AI assistance**: ~$[X]/hour equivalent productivity

## Step 7: Calculate cursor ROI — Value Per cursor Hour

This is the most important metric for understanding AI-assisted development efficiency. It answers: **"What did each hour of cursor's actual working time produce?"**

### 7a: Determine Actual cursor Clock Time
...
5. **Estimate session duration**: each session ≈ 1-4 hours of active cursor work (use commit density as signal — many commits = longer session)
...
- Assume cursor writes 200-500 lines of meaningful code per hour (much faster than humans)
- cursor active hours ≈ Total LOC ÷ 350

### 7b: Calculate Value per cursor Hour

```
Value per cursor Hour = Total Code Value (from Step 5) ÷ Estimated cursor Active Hours
```

Calculate across scenarios:

| Code Value Scenario | cursor Hours (est.) | Value per cursor Hour |
|--------------------|--------------------|-----------------------|
...
### 7c: cursor Efficiency vs. Human Developer

**Speed Multiplier**:
```
Speed Multiplier = Human Dev Hours ÷ cursor Active Hours
```

Example: If a human would need 500 hours but cursor did it in 20 hours → 25× faster

**Cost Efficiency**:
```
Human Cost = Human Hours × $150/hr
cursor Cost = Subscription ($20-200/month) + API costs (estimate from project size)
Savings = Human Cost - cursor Cost
ROI = Savings ÷ cursor Cost
```
...
**cursor Active Hours Estimate**:
- Total sessions identified: [X] sessions
- Estimated active hours: [X] hours
- Method: [git clustering / file timestamps / LOC estimate]

**Value per cursor Hour**:

| Value Basis | Total Value | cursor Hours | $/cursor Hour |
|-------------|-------------|--------------|---------------|
| Engineering only | $[X] | [X] hrs | **$[X,XXX]/cursor hr** |
| Full team (Growth Co) | $[X] | [X] hrs | **$[X,XXX]/cursor hr** |

**Speed vs. Human Developer**:
- Estimated human hours for same work: [X] hours
- cursor active hours: [X] hours
- **Speed multiplier: [X]×** (cursor was [X]× faster)

**Cost Comparison**:
- Human developer cost: $[X] (at $150/hr avg)
- Estimated cursor cost: $[X] (subscription + API)
- **Net savings: $[X]**
- **ROI: [X]×** (every $1 spent on cursor produced $[X] of value)

**The headline number**: *cursor worked for approximately [X] hours and produced the equivalent of $[X] in professional development value — roughly **$[X,XXX] per cursor hour**.*
