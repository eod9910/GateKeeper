...
**Created:** 2026-04-16  
**Status:** ACTIVE  
**Source-of-truth role:** companion to `repo-state-recovery-and-guardrails-plan.md` — captures the actions taken on 2026-04-16 and the items that still need a human decision.
...
It exists because the `repo-state-recovery-and-guardrails-plan.md` is a strategic plan; this is the running operational checklist that points at it.
...
- Modified source files across backend/src, backend/services, frontend/public, workspace — all real work in progress on **valuation regime** + **consumer cycle** + **ledger hydration** initiatives.
...
### Open issue 1 — Scope sprawl beyond the declared plan

The current authoritative plan (`ledger-data-foundation-and-pit-ingestion-plan.md`) gates further ingestion behind:
...
- **A) Promote.** Update `ledger-data-foundation-and-pit-ingestion-plan.md` to officially expand scope and supersede the "no broad scaling until contracts harden" gate. Add the new initiatives as named workstreams with their own gating criteria.
...
### Open issue 2 — OneDrive sync over multi-GB SQLite

Current footprint inside `OneDrive\Documents\Coding\pattern-detector\backend\data\`:

- `fundamentals-pit.sqlite` — **3.87 GB**
- `app-state.sqlite` — **615 MB** (with active `-wal` file)
- `symbol-catalog.sqlite` — **82 MB**

OneDrive periodically rewrites/locks files for sync. SQLite WAL mode does not tolerate that — it is a known corruption vector, and on Windows it can also trigger "file in use" failures when SQLite tries to checkpoint. With ~4.5 GB of database state in a sync folder, this is not theoretical.

**Three concrete options, ranked by effort:**

1. **Lowest effort: exclude `backend/data/` from OneDrive sync.**
   - Right-click `backend/data/` in Explorer → "Free up space" or "Always keep on this device" + add to OneDrive's per-folder exclusion (Settings → Sync and backup → Manage backup).
...
3. **Best (longer term): move large data stores out of the repo path entirely.**
   ...
   - Already partially in place — `DEFAULT_DB_PATH` constants exist in `fundamentals_pit_store.py`, `ledgerCoverage.py`, and the ledger hydration jobs. They just hardcode the in-repo path.
...
### Open issue 3 — Python/TS contract drift surface

The TS↔Python boundary is wide and there is no single canonical schema source for what crosses it.

Current crossing points:

- `pluginServiceClient.ts` ↔ `services/plugin_service.py` (strategy plugin invocation)
- `executionBridge.ts` ↔ Python execution paths (Alpaca + paper)
- `contractValidation.ts` (this is the closest thing to the canonical, but it duplicates some PIT shape definitions)
- `ledgerEngines.ts` ↔ `ledger_hydration*.py` / `ledgerContext.py`
- `signalScanner.ts` ↔ `patternScanner.py` + family/structural plugins
- New: `symbolCatalog.ts` ↔ `symbol_catalog_db.py` and `consumerCycleService.ts` ↔ `*.py` analogs
...
**No execution required this session** — but the next architectural unit of work should be a small one-pager `docs/ts-python-contract-policy.md` that picks a single rule, e.g.:

- "all crossing shapes are defined in JSON Schema under `backend/contracts/`, generated into TS types via `json-schema-to-typescript` and consumed in Python via `pydantic`-from-schema" — or
...
- If score is RED with no leakage entries → it's scope, not hygiene. Apply Issue 1 decision.
- If `backend/data` entries appear in status that aren't in `.gitignore` yet → add them here and to `.gitignore` immediately, do not let them sit.
