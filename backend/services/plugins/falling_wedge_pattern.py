#!/usr/bin/env python3
"""
Falling Wedge pattern — Bulkowski-style bullish-biased pattern.

Geometry:
  - Multiple HIGH pivots forming a falling resistance line (negative slope).
  - Multiple LOW pivots forming a less-steep falling support line (negative slope).
  - Lines converge into the future (low slope > high slope; high steeper down).
  - Confirmation = close above the upper (resistance) trendline.

Bulkowski "Encyclopedia of Chart Patterns" reference values (upward breakout):
  - Failure rate: ~11%
  - Average rise: ~32%
  - Throwback rate: ~56%
  - Forms in either downtrend (reversal) or uptrend (continuation); bullish bias.
"""
...
BULKOWSKI_AVG_RISE_PCT = 0.32
BULKOWSKI_THROWBACK_RATE = 0.56


def _evaluate_falling_wedge(
    *,
    data: List[OHLCV],
    ...
    confirm_close_above_resistance: bool,
    ...
    is_falling_bottom = low_slope_class == "falling"
    converges = lines_converge(high_fit["slope"], low_fit["slope"])
    high_steeper_down = float(high_fit["slope"]) < float(low_fit["slope"])
    ...
    if confirm_close_above_resistance:
    ...
    rules = [
        ...
        build_rule("falling_support_bottom", is_falling_bottom, low_slope_class, "falling"),
        build_rule(
            "trendlines_converge",
        ...
        build_rule("high_slope_steeper_down", high_steeper_down, "yes" if high_steeper_down else "no", "required"),
    ...
    if confirmation_index is not None:
        markers.append(marker(data, int(confirmation_index), "aboveBar", "#a855f7", "arrowUp", "BOS"))

    overlays = [
        trendline_polyline(
            data, high_fit["slope"], high_fit["intercept"],
            int(window[0]["index"]), len(data) - 1,
            "#ef4444", "Resistance (falling, steeper)", line_width=2,
        ),
        trendline_polyline(
            data, low_fit["slope"], low_fit["intercept"],
            int(window[0]["index"]), len(data) - 1,
            "#22c55e", "Support (falling)", line_width=2,
        ),
        horizontal_line(
    ...
    return {
        ...
        "reason": "falling_wedge_confirmed" if confirmation_index is not None else "falling_wedge_forming",
        "features": {
            ...
            "high_steeper_down": high_steeper_down,
...
def run_falling_wedge_pattern_plugin(
    data: List[OHLCV],
    structure: Any,
    spec: Dict[str, Any],
    symbol: str,
    ...
    **kwargs: Any,
    ...
    confirm_close_above_resistance = as_bool(setup.get("confirm_close_above_resistance", True), default=True)
    ...
    for start in range(0, len(pivots) - pivot_window_size + 1):
        result = _evaluate_falling_wedge(
            data=data,
            pivots=pivots,
            start=start,
            ...
            confirm_close_above_resistance=confirm_close_above_resistance,
    ...
    strategy_version_id = spec.get("strategy_version_id", "falling_wedge_pattern_v1")
    ...
    for idx, result in enumerate(deduped):
        ...
        candidates.append(
            build_candidate(
                ...
                pattern_type="falling_wedge_pattern",
