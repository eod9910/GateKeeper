#!/usr/bin/env python3
"""
Double Bottom primitive — Bulkowski-style classical bullish reversal pattern.

Mirror of double_top_pattern: detects two roughly equal valleys separated by
a peak, in a prior downtrend, followed (or about to be followed) by a close
above the peak high (the neckline).

Bulkowski "Encyclopedia of Chart Patterns" reference values (Adam & Adam form):
  - Failure rate (waiting for confirmation): ~4%
  - Average rise: ~38%
  - Throwback rate: ~64%
  - Best: confirmed on a close above the highest peak between the valleys

Detection pipeline mirrors double_top: 3-pivot LOW-HIGH-LOW window, Bulkowski
rule checklist, optional confirmation, score, dedupe, candidate emission.
...
def _evaluate_double_bottom(
    *,
    ...
    if len(window) != 3:
        return None
    if [p["type"] for p in window] != ["LOW", "HIGH", "LOW"]:
        return None
    ...
    valley_delta = pct_diff(v1_price, v2_price)
    valleys_symmetric = valley_delta <= valley_symmetry_pct

    lower_valley = min(v1_price, v2_price)
    peak_height = (peak_price - lower_valley) / max(lower_valley, 1e-9)
    peak_high_enough = peak_height >= peak_height_min_pct

    bars_between = int(v2["index"]) - int(v1["index"])
    spacing_ok = bars_between >= min_bars_between_valleys
    ...
    if confirm_close_above_peak:
        confirmation_index = find_close_break_index(
            data,
            start_index=int(v2["index"]) + 1,
            threshold=peak_price,
            direction="above",
        )
    ...
    rules = [
        build_rule("valleys_roughly_equal", valleys_symmetric, round(valley_delta, 4), valley_symmetry_pct),
        build_rule("peak_high_enough", peak_high_enough, round(peak_height, 4), peak_height_min_pct),
        build_rule("min_bars_between_valleys", spacing_ok, bars_between, min_bars_between_valleys),
    ...
    score = (
        0.30 * clamp01(1.0 - (valley_delta / max(valley_symmetry_pct, 1e-9)))
        + 0.25 * clamp01(peak_height / max(peak_height_min_pct * 2.0, 1e-9))
        + 0.15 * clamp01(bars_between / max(min_bars_between_valleys * 3.0, 1.0))
    ...
    markers = [
        marker(data, int(v1["index"]), "belowBar", "#22c55e", "circle", f"DB1 {v1_price:.0f}"),
        marker(data, int(v2["index"]), "belowBar", "#22c55e", "circle", f"DB2 {v2_price:.0f}"),
        marker(data, int(peak["index"]), "aboveBar", "#3b82f6", "square", f"P {peak_price:.0f}"),
    ]
    if confirmation_index is not None:
        markers.append(
    ...
    overlays = [
        line(data, [v1, peak, v2], "#22c55e", "Double Bottom Structure", line_width=2),
        horizontal_line(data, int(v1["index"]), len(data) - 1, breakout_price, "#3b82f6", "Neckline", line_style=2),
        horizontal_line(data, int(v2["index"]), len(data) - 1, target_price, "#22c55e", "Target", line_style=2),
    ...
    return {
        ...
        "reason": "double_bottom_confirmed" if confirmation_index is not None else "double_bottom_forming",
        "features": {
            "pattern_direction": "bullish",
            "valley_delta_pct": round(valley_delta, 4),
            "peak_height_pct": round(peak_height, 4),
            "bars_between_valleys": bars_between,
            "neckline_price": round(breakout_price, 4),
            "target_price": round(target_price, 4),
            "stop_price": round(lower_valley, 4),
...
def run_double_bottom_pattern_plugin(
    data: List[OHLCV],
    structure: Any,
    spec: Dict[str, Any],
    symbol: str,
    ...
    **kwargs: Any,
    ...
    peak_height_min_pct = float(setup.get("peak_height_min_pct", 0.10))
    min_bars_between_valleys = max(1, int(setup.get("min_bars_between_valleys", 10)))
    ...
    for start in range(0, len(pivots) - 2):
        result = _evaluate_double_bottom(
            data=data,
            pivots=pivots,
            start=start,
    ...
    strategy_version_id = spec.get("strategy_version_id", "double_bottom_pattern_v1")
    ...
    for idx, result in enumerate(deduped):
        ...
        candidates.append(
            build_candidate(
                ...
                pattern_type="double_bottom_pattern",
