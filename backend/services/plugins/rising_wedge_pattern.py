#!/usr/bin/env python3
"""
Rising Wedge pattern — Bulkowski-style bearish-biased pattern.

Geometry:
  - Multiple HIGH pivots forming a rising resistance line (positive slope).
  - Multiple LOW pivots forming a steeper rising support line (positive slope).
  - Lines converge into the future (low slope > high slope; both positive).
  - Confirmation = close below the lower (support) trendline.

Bulkowski "Encyclopedia of Chart Patterns" reference values (downward breakout):
  - Failure rate: ~24%
  - Average decline: ~14%
  - Pullback rate: ~63%
  - Forms in either uptrend (reversal) or downtrend (continuation); bearish bias.
"""
...
BULKOWSKI_FAILURE_RATE = 0.24
BULKOWSKI_AVG_DECLINE_PCT = 0.14
BULKOWSKI_PULLBACK_RATE = 0.63


def _evaluate_rising_wedge(
    *,
    data: List[OHLCV],
    ...
    confirm_close_below_support: bool,
    ...
    is_rising_top = high_slope_class == "rising"
    is_rising_bottom = low_slope_class == "rising"
    converges = lines_converge(high_fit["slope"], low_fit["slope"])
    low_steeper_than_high = float(low_fit["slope"]) > float(high_fit["slope"])
    ...
    if confirm_close_below_support:
    ...
    rules = [
        build_rule("rising_resistance_top", is_rising_top, high_slope_class, "rising"),
        ...
        build_rule("low_slope_steeper_than_high", low_steeper_than_high, "yes" if low_steeper_than_high else "no", "required"),
    ...
    overlays = [
        trendline_polyline(
            ...
            "#ef4444", "Resistance (rising)", line_width=2,
        ),
        trendline_polyline(
            data, low_fit["slope"], low_fit["intercept"],
            int(window[0]["index"]), len(data) - 1,
            "#22c55e", "Support (rising, steeper)", line_width=2,
        ),
        horizontal_line(
            data, int(window[0]["index"]), len(data) - 1, target_price,
            "#ef4444", "Target", line_style=2,
        ),
    ]
    ...
    return {
        ...
        "reason": "rising_wedge_confirmed" if confirmation_index is not None else "rising_wedge_forming",
        "features": {
            ...
            "low_steeper_than_high": low_steeper_than_high,
...
def run_rising_wedge_pattern_plugin(
    data: List[OHLCV],
    structure: Any,
    spec: Dict[str, Any],
    symbol: str,
    ...
    **kwargs: Any,
    ...
    confirm_close_below_support = as_bool(setup.get("confirm_close_below_support", True), default=True)
    ...
    for start in range(0, len(pivots) - pivot_window_size + 1):
        result = _evaluate_rising_wedge(
            ...
            confirm_close_below_support=confirm_close_below_support,
    ...
    strategy_version_id = spec.get("strategy_version_id", "rising_wedge_pattern_v1")
    ...
    for idx, result in enumerate(deduped):
        ...
        candidates.append(
            build_candidate(
                ...
                pattern_type="rising_wedge_pattern",
